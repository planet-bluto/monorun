...
const weapons = {
	- 1:{
		"obj":preload("res://objects/Weapon.tscn"), 
		"cooldown":40
	}
}
...
remotesync func spawn_proj(w_type, angle, echo = true):
	if (weapons.has(w_type) and w_cooldown == 0):
		...
		if (echo): rpc("spawn_proj", w_type, angle, false)
		if (type == 0): proj_id += 1
		w_inst.inited = true



remotesync func damage(amount, echo = true):
	var valid = false
	...
	if (echo == false):
		...
		if (Network.active):
			if (Network.id != id):
				if (echo): rpc("damage", amount, false)
		else:
			valid = true
...
func get_aim_angle():
	return atan2(mouse_screen_pos.y - screen_pos.y, mouse_screen_pos.x - screen_pos.x)
...
func _process(_delta):
	...
	mouse_screen_pos = (get_viewport().get_mouse_position()-(OS.window_size/2.0))+global_position
